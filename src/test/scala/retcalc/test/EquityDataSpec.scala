...
import equity_inflation_data.EquityData
...
class EquityDataSpec extends AnyWordSpec with Matchers with TypeCheckedTripleEquals {
  ...
  "EquityData.fromResource" should {
    "return the expected collection of EquityData case classes" in {
      val path = "sp500_2017.tsv"
      EquityData.fromResource(path) should === (expectedEquityData)
    }
  }
